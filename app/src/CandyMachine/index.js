...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getCandyMachineState = async () => {
    ...
    // Fetch the metadata from our candy machine
    const candyMachine = await program.account.candyMachine.fetch(
      process.env.REACT_APP_CANDY_MACHINE_ID
    );

    console.log("Candy Machine Metadata: ", candyMachine);
    ...
    // Fetch all the mints that have been made
    const data = await fetchHashTable(
      process.env.REACT_APP_CANDY_MACHINE_ID,
      true
    );

    console.log("Hash Table Data: ", data);
  ...
  const fetchHashTable = async (hash, metadataEnabled) => {
    ...
    console.log("Metadata Accounts: ", metadataAccounts);
  ...
  const renderDropTimer = () => {
    ...
    // Else return the current drop date
